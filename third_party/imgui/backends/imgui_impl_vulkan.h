...
//  identifier. Call ImGui_ImplVulkan_AddTexture() to register one. Read the
//  FAQ about ImTextureID/ImTextureRef +
//  https://github.com/ocornut/imgui/pull/914 for discussions. [X] Renderer:
//  Large meshes support (64k+ vertices) even with 16-bit indices
...
// with your custom function. If you have no idea what this is, leave it alone!
// #define IMGUI_IMPL_VULKAN_NO_PROTOTYPES

// [Configuration] Convenience support for Volk
// (you can also technically use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk
// via ImGui_ImplVulkan_LoadFunctions().) (When using Volk from directory
// outside your include directories list you can specify full path to the
// volk.h header,
...
struct ImGui_ImplVulkan_PipelineInfo {
    ...
    VkImageUsageFlags
        SwapChainImageUsage;  // Extra flags for vkCreateSwapchainKHR() calls
                              ...
                              // add e.g. VK_IMAGE_USAGE_TRANSFER_SRC_BIT if
                              // you need to capture from viewports.
};
...
struct ImGui_ImplVulkan_InitInfo {
    ...
    VkQueue          Queue;
    VkDescriptorPool
        DescriptorPool;  // See requirements in note above; ignored if using
                         // DescriptorPoolSize > 0
    uint32_t DescriptorPoolSize;    // Optional: set to create internal
                                    // descriptor pool automatically instead of
    ...
    // Need to explicitly enable VK_KHR_dynamic_rendering extension to use
    // this, even for Vulkan 1.3 + setup
    // PipelineInfoMain.PipelineRenderingCreateInfo and
...
// Follow "Getting Started" link and check examples/ folder to learn about
// using backends!
...
// VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR)) Else, the pipeline
// can be created, or re-created, using ImGui_ImplVulkan_CreateMainPipeline()
// before rendering.
IMGUI_IMPL_API void
ImGui_ImplVulkan_CreateMainPipeline(const ImGui_ImplVulkan_PipelineInfo* info);
...
ImGui_ImplVulkan_AddTexture(VkSampler     sampler,
...
// This is temporarily stored in GetPlatformIO().Renderer_RenderState during
// the ImGui_ImplVulkan_RenderDrawData() call. (Please open an issue if you
...
// (swap chain, render pass, frame buffers, etc.). You may read this code if
// you are curious, but it is recommended you use your own custom tailored code
// to do equivalent work.
//
// The ImGui_ImplVulkanH_XXX functions should NOT interact with any of the
// state used by the regular ImGui_ImplVulkan_XXX functions.
//-------------------------------------------------------------------------
...
// Helpers
IMGUI_IMPL_API void
ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance       instance,
...
IMGUI_IMPL_API void
ImGui_ImplVulkanH_DestroyWindow(VkInstance                   instance,
                                VkDevice                     device,
                                ImGui_ImplVulkanH_Window*    wd,
                                const VkAllocationCallbacks* allocator);
...
// Helper structure to hold the data needed by one rendering context into one
// OS window (Used by example's main.cpp. Used by multi-viewport features.
...
struct ImGui_ImplVulkanH_Window {
    ...
    uint32_t ImageCount;  // Number of simultaneous in-flight frames (returned
                          ...
                          // min_image_count)
    uint32_t SemaphoreCount;  // Number of simultaneous in-flight frames + 1,
                              // to be able to use it in vkAcquireNextImageKHR
    uint32_t
